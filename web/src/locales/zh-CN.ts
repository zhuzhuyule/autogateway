export default {
  ...
  keys: {
    ...
    editWeight: "编辑权重",
    editWeightAndPriority: "编辑权重与优先级",
    ...
    priorityHint:
      "选路层级:数值越小越优先。该层全部不可用(熔断/无可用密钥)时才降到下一层;同层内按权重分配。默认 100。",
    weightCannotBeNegative: "权重不能为负数",
    priorityCannotBeNegative: "优先级不能小于 1",
    weightMaxExceeded: "权重不能超过1000",
    priorityMaxExceeded: "优先级不能超过 1000",
    newWeight: "新权重",
    newPriority: "新优先级",
    currentWeight: "当前权重",
    priority: "优先级",
    ...
    multiUpstreamTooltip:
      "同一家 provider 的多个等价 base URL,按权重分流(自建集群/多副本/主备). 90% 场景一个就够,默认隐藏权重和添加按钮.",
    ...
    upstreamTooltip:
      "API服务器的完整URL地址。多个上游可以实现负载均衡和故障转移，提高服务可用性。末尾 /v1 / /v1beta 等版本段会被自动识别,带不带都行。",
    weightTooltip:
    ...
    currentGroup: "当前分组",
    invalidGroup: "无效的分组，无法保存",
    invalidAggregateGroup: "无效的聚合分组，无法保存",
    enterKeysPlaceholder: "输入密钥，每行一个；可用 “key,备注” 为单个密钥加标签",
    labelFormatHint:
      "每行一个密钥。可选：在密钥后加英文逗号和备注（如 sk-xxx,生产环境）为该密钥设置标签。",
    editLabelTip: "点击编辑标签",
    addLabel: "加标签",
    addLabelPlaceholder: "给这个密钥起个名…",
    deleteKeysWarning: "删除后密钥将从该分组中永久移除，此操作不可恢复。",
  ...
  settings: {
    ...
    backup: {
      ...
      replaceWarn:
        "替换模式将删除 {groups} 个分组、{keys} 个密钥、{aliases} 个别名。请在下方输入 DELETE 确认。",
  ...
  aliases: {
    // 原"快速整理"tab 的入口, 现在收进主 tab 的一个弹窗
    browseFamily: "按家族整理",
    // auto 智能路由面板的状态说明: 关掉之后不是"不路由", 而是一律走 simple。
    auto: {
      modeThreshold: "按上下文长度分档",
      modeSimple: "已关闭：所有 auto 请求一律走 simple 档",
    },
    // 别名列表 —— 一行 = 一个别名, 候选收在抽屉里。
    list: {
      titleMeta: "{aliases} 个别名 · {candidates} 条候选",
      search: "搜索别名 / 分组 / 模型",
      filterAll: "全部状态",
      filterProblem: "仅有问题 ({n})",
      filterAuto: "仅智能路由档位",
      filterAnyGroup: "全部分组",
      ...
      edit: "编辑",
      empty: "没有匹配的别名。",
      problemNoCandidates: "无候选",
      problemUnexposed: "{n} 个未公开",
      problemBlocked: "{n} 个被拉黑",
      problemDisabled: "{n} 个已停用",
    },
    // 详情抽屉 —— 编辑单位是整个别名, 页脚给窗口内实测。
    drawer: {
      window: "24h 调用",
      errRate: "错误率",
      avgMs: "平均耗时",
      ...
      expose: "公开",
      exposeTip:
        "分组是 specified 模式但该模型不在暴露列表里, 选路会跳过它 —— 点一下补进 exposed_models。",
      exposeDone: "{model} 已加入分组暴露列表",
      exposeAlreadyOk: "{model} 本来就在暴露列表里",
    ...
    edit: {
      ...
      moveUp: "上移",
      moveDown: "下移",
      namePlaceholder: "别名名称",
      orderHint: "拖拽顺序即偏好顺序 —— 仅在候选权重相同时决定先后, 不会改变加权分配。",
      enabledOn: "参与选路",
      enabledOff: "已停用, 不参与选路",
      empty: "还没有候选。从下面选一个分组 + 模型加进来。",
      pickGroup: "选择分组",
      pickModel: "选择模型",
      add: "添加",
      needGroupAndModel: "请先选择分组和模型",
      duplicateCandidate: "这个分组下已经有该模型了",
      needAtLeastOne: "至少要保留一个候选",
      discardTitle: "放弃未保存的改动?",
      discardConfirm: "抽屉里的改动还没保存, 关闭后会丢失。",
      discard: "放弃",
      deleteAlias: "删除别名",
      deleteTitle: "删除这个别名?",
      deleteConfirm: "将移除 {alias} 的全部 {n} 个候选, 该别名将不再可用。",
      deletedTitle: "已删除 {alias}",
      deletedUndo: "{n} 个候选已移除。",
      undo: "撤销",
      undone: "已恢复",
    },
    quick: {
  ...
  v3: {
    ...
    modePassthrough: "透传",
    modeSpecified: "指定",
    modePassthroughHint: "透传 = 放行:上游声明的模型全部对外可调,不做白名单筛选",
    modeSpecifiedHint:
      "指定 = 白名单:仅「已暴露」段的模型对外可调,可拖拽排序、加别名;下方「上游全部」是候选池",
    blocked: "黑名单",
    block: "加入黑名单",
    unblock: "解除拉黑",
    exposedModels: "已暴露",
    exposedHint: "对外可调的白名单 — 仅这些模型能被调用,可拖拽排序、加别名",
    upstreamAll: "未暴露",
    upstreamHint: "上游里尚未暴露的模型 — 从这里加入上方「已暴露」白名单",
    upstreamEmpty: "上游模型列表为空,请先点击刷新",
    noExposedModels: "暂无已暴露模型,从下方加入",
    allExposed: "上游模型已全部暴露,无可添加项",
    addToExposed: "加入",
    alreadyExposed: "已加入",
    testModel: "测试连通性",
    ...
    testModalityAuto: "自动识别",
    testViaModality: "按 {mode}",
    testModalityChat: "文本 Chat",
    testModalityImage: "图像生成",
    testModalityVision: "视觉输入",
    testModalityTts: "语音合成 TTS",
    testModalityAsr: "语音识别 ASR",
    testModalityTip:
      "选择测试模态: 自动识别按模型能力(图像生成/视觉/文本)分别构造探活请求, 或强制指定一种",
    probeShortChat: "文本",
    probeShortImage: "图像",
    probeShortVision: "视觉",
    probeShortTts: "合成",
    probeShortAsr: "识别",
    probeChipAuto: "探针形态：{mode}（按模型能力自动识别，点击可固定为某一种）",
    probeChipManual: "探针形态：{mode}（已手动固定，点击可恢复自动识别）",
    ...
    aliasUnexposed: "失效",
    aliasUnexposedTip:
      "目标分组处于「指定」模式且 {model} 不在已暴露列表中,该 alias 路由时会被跳过。前往该分组的「模型」标签页加入此模型即可恢复。",
    ...
    modelCopied: "已复制",
    aliasAvgMsTip: "全程耗时 · 24h 平均(含流式回答时长)",
    aliasCostTip: "折算成本 · 24h(免费源显示用量 token)",
    // 候选状态标签: 区分"为什么这条候选不参与选路"。
    aliasStateUsable: "可用",
    aliasStateDisabled: "已停用",
    aliasStateUnexposed: "未暴露",
    aliasStateBlocked: "已拉黑",
    aliasUnusableN: "不可用 ({n})",
    aliasFix: "修复",
    aliasDisabledTip: "该候选已被手动停用, 不会参与选路",
  },
  v5: {
    // Aliases 视图 — 未识别模型建议横幅
    suggestionsTitle: "近期请求中检测到未识别模型 — 点击为其建立别名",
    suggestionsDismiss: "本次会话不再提示",
    suggestionsEmpty: "暂时没有可建议的别名。",
  ...
  sync: {
    ...
    identityHint:
      "把上面两项发给对端管理员: URL 填进对端的「节点 URL」, 指纹填进「对端身份指纹」. 私钥永不离开本机.",
    ...
    pinnedFingerprintHint:
      "对端在 UI 上展示的「本机指纹」, 填进来后我们会在握手时校验对端真实公钥的指纹是否一致, 不一致即拒绝连接. 留空 = 首次连接信任 (TOFU).",
    status: "状态",
    statusConnected: "已连接",
  ...
  upgrade: {
    ...
    confirmTitle: "确认升级",
    confirmBody:
      "即将向宿主机 watcher 写入升级请求: 从 {from} → {to}. 真正升级由宿主机外部 watcher 接管, 进程零特权.",
    confirmYes: "确认升级",
  ...
  playground: {
    ...
    fileProtocolWarning:
      "当前页面是通过 file:// 直接打开的, 所有后端请求 (/api、/proxy) 都无法发出 — 图像生成、模型列表等都会失败。请通过 api-center 后端地址访问 (如 http://localhost:3001/)。",
    invalidJsonResponse: "上游返回了无法解析为 JSON 的响应",
    recentImages: "最近图片",
    uploadNew: "从本地上传",
