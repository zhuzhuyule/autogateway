export default {
  ...
  keys: {
    ...
    priorityHint: "Routing tier: lower wins. The next tier is only used when every sub-group in this tier is unavailable (circuit-open / no active keys). Within a tier, traffic is split by weight. Default 100.",
    ...
    labelFormatHint: "One key per line. Optional: add a comma and note after a key (e.g. sk-xxx,production) to label it.",
    editLabelTip: "Click to edit label",
    addLabel: "Add label",
    addLabelPlaceholder: "Name this key…",
    deleteKeysWarning: "The keys below will be permanently removed from this group. This action cannot be undone.",
  ...
  settings: {
    ...
    backup: {
      ...
      replaceWarn: "Replace will delete {groups} groups, {keys} keys, {aliases} aliases. Type DELETE below to confirm.",
  ...
  aliases: {
    ...
    quick: {
      ...
      errors: {
        DUPLICATE_RESOURCE: "Alias already maps one of the selected models — uncheck duplicates and retry",
  ...
  v3: {
    ...
    modePassthroughHint: "Passthrough = allow all: every model the upstream declares is callable — no whitelist filtering",
    modeSpecifiedHint: "Specified = whitelist: only models in the “Exposed” section are callable, reorderable and aliasable; “Upstream” below is the candidate pool",
    blocked: "Blocked",
    block: "Block",
    unblock: "Unblock",
    exposedModels: "Exposed",
    exposedHint: "Caller-facing whitelist — only these models can be called; drag to reorder; aliasable",
    ...
    testModalityTip: "Test modality: auto-detect builds per-model probes by capability (image generation / vision / text), or force one shape",
    probeShortChat: "chat",
  ...
  sync: {
    ...
    enable: "Enable Sync",
    enableHint: "When on, all configuration (groups / aliases / API keys) is mirrored to every peer below in real time. No per-table toggles needed.",
    syncSecret: "Global Encryption Key",
    syncSecretHint: "All nodes must share the exact same key. Payloads are AES-GCM encrypted end-to-end — a compromised network path cannot decrypt them.",
    ...
    identityHint: "Share both lines with the peer admin: paste URL into their 'Peer URL', paste fingerprint into 'Expected Peer Fingerprint'. Private key never leaves this machine.",
