export default {
  ...
  keys: {
    ...
    priorityHint: "ルーティング階層:数値が小さいほど優先されます。この階層が全て利用不可(サーキットオープン/有効なキーなし)のときのみ次の階層に降ります。同一階層内はウェイトで分配されます。既定値は 100。",
    ...
    labelFormatHint: "一行に一つのキー。任意：キーの後にカンマと備考を付けるとラベルになります（例：sk-xxx,本番）。",
  ...
  settings: {
    ...
    backup: {
      ...
      replaceWarn: "置換すると {groups} グループ、{keys} キー、{aliases} エイリアスが削除されます。下に DELETE と入力して確認してください。",
  ...
  aliases: {
    ...
    quick: {
      ...
      panelHeader: "既存の別名",
      errors: {
        DUPLICATE_RESOURCE: "同じモデルマッピングが既に存在します。重複をはずして再送信してください",
  ...
  v3: {
    ...
    noTemplateForChannel: "このチャネルのテンプレート未収録 — フォーム直入力でカスタム Provider を作成可能",
    optional: "任意",
    ...
    getKey: "Key 取得",
    newGroupFootHint:
      "Provider 追加後、詳細画面で Key を追加・モデル到達性をテストできます。",
    docs: "ドキュメント",
    editNotes: "メモを編集",
    ...
    aliasPendingHint: "下のモデルの + をクリックして一時追加。グループを跨いで複数選択し、最後に一括確定できます",
    ...
    unblock: "ブラック解除",
    exposedModels: "公開中",
    exposedHint: "外部から呼び出せるホワイトリスト — 呼び出し可能なモデルはこの一覧のみ。ドラッグで並べ替え、別名を追加できます",
    upstreamAll: "未公開",
    upstreamHint: "まだ公開していない上流モデル — ここから上の「公開中」ホワイトリストに追加できます",
    ...
    testModalityTip: "テストモダリティの選択: 自動認識はモデルの能力（画像生成/ビジョン/テキスト）に応じて生存確認リクエストを組み立てます。1 つに手動固定も可能",
    probeShortChat: "テキスト",
    probeShortImage: "画像",
    probeShortVision: "ビジョン",
    probeShortTts: "合成",
    probeShortAsr: "認識",
    probeChipAuto: "プローブ形態: {mode}（モデルの能力から自動認識、クリックで 1 つに固定できます）",
    ...
    alias: "別名",
    aliasUnexposed: "無効",
    aliasUnexposedTip: "ターゲットグループが「指定」モードで {model} が公開中リストに含まれていないため、この別名はルーティング時にスキップされます。そのグループの「モデル」タブでこのモデルを追加すると復元できます。",
    routingPersistDrift: "バックエンドが新しいフィールドを認識していません — 再ビルドしてサービスを再起動してください。変更はローカルにのみ保存されています",
    smartDetect: "スマート認識",
  ...
  v5: {
    ...
    maAddTitle: "別名を追加",
    maAddHint: "Enter または + クリックで追加; 既存の再利用 = 現在のモデルをその別名の候補プールに追加",
    ...
    testModalityTts: "音声合成 TTS",
    testModalityAsr: "音声認識 ASR",
    testModalityTip: "テスト形態: 自動判別はモデル能力(画像生成/画像入力/テキスト)に応じた探活リクエストを作成、手動で一種に固定も可能",
  ...
  sync: {
    peerSync: "マルチノード同期",
    enable: "同期を有効化",
    enableHint: "有効化すると、下記のすべての Peer と設定(グループ/別名/API キー)をリアルタイムに双方向同期します。個別オプションは不要です。",
    syncSecret: "グローバル暗号鍵",
    syncSecretHint: "全ノードで同一の鍵を設定してください。ペイロードは AES-GCM でエンドツーエンド暗号化され、経路漏洩でも解読不能です。",
    ...
    identityHint: "上記2項目を対向ノード管理者に共有: URL を「ノード URL」に、フィンガープリントを「対向ノード期待フィンガープリント」に貼り付け. 秘密鍵は本機を離れない.",
