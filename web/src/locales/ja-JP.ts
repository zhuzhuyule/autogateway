export default {
  ...
  sync: {
    ...
    syncKey: "同期キー (Peer Token)",
    syncKeyHint: "相手ノードの SyncPeer に登録された同一のトークン",
    syncApiKeys: "API Keys を同期",
    apiKeysWarningTitle: "危険: 全 API Keys のノード間同期",
    apiKeysWarningBody:
      "本ノードの API Keys がすべて暗号化されて相手ノードに送信されます。相手ノードや SyncKey が漏洩した場合、すべての上流クレデンシャルが露出します。完全に信頼できる自身管理下のノード間のみで有効にしてください。",
    confirmPhrasePrompt: "保存を有効にするには下記の確認フレーズを入力:",
    confirmPhraseError: "確認フレーズが一致しません — 保存はブロックされました",
    status: "状態",
