...
var MessagesZhCN = map[string]string{
	...
	"config.force_stream_usage":           "强制流式返回用量",
	"config.force_stream_usage_desc":      "对 OpenAI 格式的流式对话，注入 stream_options.include_usage，让上游在流尾回传 token 用量帧（成本统计需要）。会给客户端多发一帧 usage；若客户端不兼容请关闭。",
	"config.enable_llm_error_triage":      "LLM 错误归因兜底",
	"config.enable_llm_error_triage_desc": "上游报错时，若内置规则判不出这是「客户端请求错」还是「key/账号错」，就请一个便宜模型二次判定，避免把请求错（如音频格式非法）误算到 key 头上熔断好 key。默认关：仅疑难错才触发，会消耗少量 token；同种错按签名缓存只问一次；判定失败一律回退保守（照旧计入）。",
	"config.llm_error_triage_group":       "错误归因分组",
	"config.llm_error_triage_group_desc":  "开启 LLM 错误归因后，用哪个分组来跑归因判定。须为 OpenAI 兼容分组（走其 /chat/completions），建议指向一个便宜或免费的模型分组。留空则不启用兜底。",
	"config.llm_error_triage_model":       "错误归因模型",
	"config.llm_error_triage_model_desc":  "归因判定使用的模型名（在上面所选分组内）。选便宜、快的小模型即可，判定只需回一个词。",
