...
var MessagesEnUS = map[string]string{
	...
	"config.force_stream_usage":           "Force stream token usage",
	"config.force_stream_usage_desc":      "For OpenAI-format streaming chat, inject stream_options.include_usage so upstream returns a token-usage frame (needed for cost tracking). Adds one extra usage frame to the client stream; leave off if your client rejects it.",
	"config.enable_llm_error_triage":      "LLM error triage",
	"config.enable_llm_error_triage_desc": "When an upstream error can't be classified by the built-in rules as either a client request error or a key/account error, ask a cheap model to adjudicate — so request errors (e.g. an invalid audio format) don't get counted against the key and blacklist a healthy key. Off by default: only fires on ambiguous errors, spends a few tokens, caches each distinct error by signature so it's asked once, and always falls back to the conservative default (count it) if the call fails.",
	"config.llm_error_triage_group":       "Error triage group",
	"config.llm_error_triage_group_desc":  "Which group runs the triage call when LLM error triage is on. Must be an OpenAI-compatible group (its /chat/completions is used); point it at a cheap or free model group. Leave empty to disable the fallback.",
	"config.llm_error_triage_model":       "Error triage model",
	"config.llm_error_triage_model_desc":  "Model name (within the group above) used for the triage call. A small, fast, cheap model is enough — it only needs to answer with a single word.",
