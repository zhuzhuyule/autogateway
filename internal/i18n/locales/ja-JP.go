...
var MessagesJaJP = map[string]string{
	...
	"config.force_stream_usage":           "ストリームの使用量を強制取得",
	"config.force_stream_usage_desc":      "OpenAI 形式のストリーミングチャットで stream_options.include_usage を注入し、上流がトークン使用量フレームを返すようにします（コスト集計に必要）。クライアントへ usage フレームが 1 つ追加されます。互換性がない場合は無効にしてください。",
	"config.enable_llm_error_triage":      "LLM エラー原因判定",
	"config.enable_llm_error_triage_desc": "上流エラーを組み込みルールで「クライアントのリクエストエラー」か「キー/アカウントのエラー」か判別できない場合、安価なモデルに二次判定させます。これにより、リクエストエラー（音声フォーマット不正など）を誤ってキーの失敗に数え、正常なキーをブラックリスト化するのを防ぎます。既定は無効：判別困難なエラーのみ発火し、少量のトークンを消費、同種のエラーは署名でキャッシュし一度だけ問い合わせ、呼び出し失敗時は常に保守的な既定（数える）にフォールバックします。",
	"config.llm_error_triage_group":       "エラー判定グループ",
	"config.llm_error_triage_group_desc":  "LLM エラー原因判定が有効なとき、判定呼び出しに使うグループ。OpenAI 互換グループである必要があり（その /chat/completions を使用）、安価または無料のモデルグループを指定してください。空にするとフォールバックは無効です。",
	"config.llm_error_triage_model":       "エラー判定モデル",
	"config.llm_error_triage_model_desc":  "上記グループ内で判定呼び出しに使うモデル名。判定は 1 単語で答えるだけなので、小さく高速で安価なモデルで十分です。",
