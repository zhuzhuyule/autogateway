...
func RunMigrateKeys(args []string) {
	...
	migrateCmd.Usage = func() {
		...
		fmt.Println()
		fmt.Println("Usage:")
		fmt.Println("  Enable encryption: autogateway migrate-keys --to new-key")
		fmt.Println("  Disable encryption: autogateway migrate-keys --from old-key")
		fmt.Println("  Change key: autogateway migrate-keys --from old-key --to new-key")
		fmt.Println()
