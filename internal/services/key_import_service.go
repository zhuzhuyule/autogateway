...
func (s *KeyImportService) StartImportTask(group *models.Group, keysText string) (*TaskStatus, error) {
	keys, notesByKey := s.KeyService.parseKeysWithNotes(keysText)
	if len(keys) == 0 {
	...
	go s.runImport(group, keys, notesByKey)

	return initialStatus, nil
}

func (s *KeyImportService) runImport(group *models.Group, keys []string, notesByKey map[string]string) {
	...
	addedCount, ignoredCount, err := s.KeyService.processAndCreateKeys(group.ID, keys, notesByKey, progressCallback)
	if err != nil {
